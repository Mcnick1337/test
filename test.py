...
def run_ollama(prompt, model):
    ...
    try:
        ...
        with open('ollama_raw_response.txt', 'w', encoding='utf-8') as f:
            f.write(response)
        raise ValueError("Could not find or parse JSON content in model response.")
...
def generate_trade_signal(df, df_1h, news_summaries, metrics, liquidity, volume_profile, multi_df, trend_info):
    ...
    prompt = f"""
You are a professional crypto trader with 5+ years of experience. Use your expertise in RSI, MACD, EMA, VWAP, ADX, volume profile, order book zones, market news, and blockchain metrics to assess crypto market conditions and generate actionable signals.

Your task:
1. Evaluate the confluence of signals.
2. Identify the scenario type: Breakout, Reversal, Trend Continuation, or Range-Bound.
3. Generate a clear trade signal in structured JSON format.

Technical Indicators:
{indicators}

Ichimoku Signal (Tenkan-sen): {ichimoku_signal}

Multi-Timeframe Indicators:
- 1m: RSI {mtf_data['1m']['RSI']}, EMA20 {mtf_data['1m']['EMA_20']}, EMA200 {mtf_data['1m']['EMA_200']}, ATR {mtf_data['1m']['ATR']}, VWAP {mtf_data['1m']['VWAP']}
- 5m: RSI {mtf_data['5m']['RSI']}, EMA20 {mtf_data['5m']['EMA_20']}, EMA200 {mtf_data['5m']['EMA_200']}, ATR {mtf_data['5m']['ATR']}, VWAP {mtf_data['5m']['VWAP']}
- 15m: RSI {mtf_data['15m']['RSI']}, EMA20 {mtf_data['15m']['EMA_20']}, EMA200 {mtf_data['15m']['EMA_200']}, ATR {mtf_data['15m']['ATR']}, VWAP {mtf_data['15m']['VWAP']}
- 1h: RSI {mtf_data['1h']['RSI']}, EMA20 {mtf_data['1h']['EMA_20']}, EMA200 {mtf_data['1h']['EMA_200']}, ATR {mtf_data['1h']['ATR']}, VWAP {mtf_data['1h']['VWAP']}
- 4h: RSI {mtf_data['4h']['RSI']}, EMA20 {mtf_data['4h']['EMA_20']}, EMA200 {mtf_data['4h']['EMA_200']}, ATR {mtf_data['4h']['ATR']}, VWAP {mtf_data['4h']['VWAP']}
- 1d: RSI {mtf_data['1d']['RSI']}, EMA20 {mtf_data['1d']['EMA_20']}, EMA200 {mtf_data['1d']['EMA_200']}, ATR {mtf_data['1d']['ATR']}, VWAP {mtf_data['1d']['VWAP']}

Liquidity Zones:
- Strongest Bid Zone: ${liquidity.get('bid_zone_price', 'N/A')} (Vol: {liquidity.get('bid_zone_volume', 'N/A')})
- Strongest Ask Zone: ${liquidity.get('ask_zone_price', 'N/A')} (Vol: {liquidity.get('ask_zone_volume', 'N/A')})
- Order Book Imbalance: {liquidity.get('bid_ask_imbalance_pct', 0):.2f}%

Volume Profile:
- High Volume Zone: {volume_profile.get('top_volume_zone', 'N/A')}
- Volume at Zone: {volume_profile.get('volume_at_top_zone', 'N/A')}

Trend Analysis:
- Direction: {trend_info.get('trend_direction', 'N/A')}
- Strength: {trend_info.get('trend_strength', 'N/A')}
- ADX: {trend_info.get('adx', 'N/A')}
- Volatility: {trend_info.get('volatility', 'N/A')}

Blockchain Metrics:
- Market Cap: ${metrics.get('market_cap_usd', 'N/A')}
- 24h Volume: ${metrics.get('volume_usd', 'N/A')}
- Bitcoin Hash Rate: {metrics.get('hash_rate', 'N/A')}
- Ethereum Nodes: {metrics.get('nodes', 'N/A')}

Recent Crypto News:
{news_input}

Reply in a JSON format with:
